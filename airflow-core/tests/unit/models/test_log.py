...
class TestLogTaskInstanceJoin:
    ...
    def test_log_task_instance_join_with_same_task_id_different_dags(self, dag_maker, session):
        ...
        # Create a log entry for dag_2's task instance
        log2 = Log(
            event="test_event_2",
            task_instance=ti2,
        )
        ...
        stmt = select(Log).where(Log.id == log1.id).options(joinedload(Log.task_instance))
        loaded_log = session.scalar(stmt)

        # Verify that the join correctly resolved to dag_1's task instance
        ...
        assert loaded_log.task_instance.state == TaskInstanceState.SUCCESS

        # Verify the join for the second log entry
        stmt2 = select(Log).where(Log.id == log2.id).options(joinedload(Log.task_instance))
        loaded_log2 = session.scalar(stmt2)

        # This should correctly resolve to dag_2's task instance, not dag_1
        ...
        assert loaded_log2.task_instance.state == TaskInstanceState.FAILED
